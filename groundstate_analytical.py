...
def plot_c(y1, y2, y3, x, index, labels, para):
    ...
    #     plt.scatter(x_values[point], y_values[point])
    #     plt.scatter(x_values[point], y_values[point], marker=labels[y_values[point]])
    plt.plot(x,y1[:,index], label=r'$J$')
    plt.plot(x,y2[:,index], label=r'$D_y$')
    plt.plot(x,y3[:,index], label=r'$\Gamma$')

    plt.xticks(x)
    plt.ylabel('energy')
    plt.xlabel('impurity separation distance in a')
    #para = [sites_x, sites_y, mu, cps, tri, gamma, jott, 0, 0] 
    plt.title('Energy of spin coefficients for different distances \n and ' +labels[index]+' spin configuration \n for N = '+str(para[0]) + r', $\mu =$ '+str(para[1])+ r', $V=$'+str(para[2])+r', $U=$'+str(para[3]) + r', $\gamma=$'+str(para[4]) + r', $J=$'+str(para[5]) )
    plt.legend()
    plt.tight_layout()
    plt.savefig(name)
    #plt.show()
...
def main(sites_x, mu, cps, tri, gamma, jott):
    ...
    for pos in tqdm(range(1,sites_x)):
        ...
        all_F, J_vec, D_y, Gamma, spin_orientations = an(pos, compare=False, plotting=False)
    ...
    plot_c(np.array(Jc), np.array(Dc), np.array(Gc), range(1,sites_x), 0, labels, parameters[:-2])
...
if __name__ == '__main__':
    ...
    if main(args.sites_x, args.chemical, args.attract, args.tri, args.rkky, args.soc): 
