...
import argparse
import plot
import routines_selfconsistent as routiness
...
def main(sites_x, sites_y, mu, cps, tri, jott, gamma, imp1, imp2):
    ...
    #### let's get to work
    if spin_opti:
        positions =[[5, 10], [5, 30], [5,50]] # [[1,2],[1,3]] #
        spin_configs, free_energies = [], []
        for pos in positions: # go through the possible positions for the impurity spins
            configs, energies = routiness.spin_loop(parameters, kvalues, pos, iteration=iter_bool) # calculate free energy for different orientations of the impurity spins
    ...
    if triplet_study:
        ...
        study_results = routiness.gap_study(parameters, [kvalues, positive_kvalues, zero_kvalues], spin_orientation, spin_positions) #[gap_studies,  energies, density_study, parameters, labels, site]
        plot.density(study_results[1], study_results[2], study_results[5], study_results[3], study_results[4], 'tri_self', output=False)
        # plot.gap(routiness.gap_eff(sites_x, study_results[0], tri, study_results[6])[1], parameters, study_results[4], 'tri')
        # plot.spinphase(study_results[8], study_results[7], parameters, study_results[4])
        print(routiness.zero_LDOS_gap(parameters, study_results[2], study_results[1], study_results[6]))

    if SOC_study:
        # for different gamma values: density, singlet gap and triplet gap
        study_results = routiness.soc_study(parameters, [kvalues, positive_kvalues, zero_kvalues], spin_orientation, spin_positions)
        plot.density(study_results[1], study_results[2], study_results[5], study_results[3], study_results[4], 'soc_self')
        # plot.gap(routiness.gap_eff(sites_x, study_results[0], gamma, study_results[6])[1], parameters, study_results[4], 'soc')
        if len(study_results[8]) > 3:
            plot.spinphase(study_results[8], study_results[7], parameters, study_results[4])

    if RKKY_study:
        # for different jott values: density, singlet gap and triplet gap
        study_results = routiness.rkky_study(parameters, [kvalues, positive_kvalues, zero_kvalues], spin_orientation, spin_positions, iteration=iter_bool)
        plot.density(study_results[1], study_results[2], study_results[5], study_results[3], study_results[4]+[element+' tri' for element in study_results[4]], 'rkky_self')
        # plot.gap(routiness.gap_eff(sites_x, study_results[0], jott, study_results[6])[1], parameters, study_results[4], 'rkky', abs=False)
    ...
    if gap_3D:
        ...
        for rkky in range(0,10,2):
            ...
            study_results = routiness.gap_study(parameters, [kvalues, positive_kvalues, zero_kvalues], spin_orientation, spin_positions) #[gap_studies,  energies, density_study, parameters, labels, site]
            plot.density(study_results[1], study_results[2], study_results[5], study_results[3], study_results[4], 'tri_self', output=False)
            gap.append(routiness.zero_LDOS_gap(parameters, study_results[2], study_results[1], study_results[6]))
